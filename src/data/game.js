...
export const areIdentical = (rowOrCol1, rowOrCol2) => {
  ...
  return (
    rowOrCol1.filter((v, i) => v === rowOrCol2[i]).length === rowOrCol1.length
  );
};
...
export const isBoardFull = board => {
  return board.reduce((sum, row) => sum + numberOfValues(row, 0), 0) === 0;
};
...
export const duplicateRows = board => {
  return board
    .map(
      (row, index) =>
        board.filter(
          (row2, index2) => index !== index2 && areIdentical(row, row2)
        ).length > 0
          ? index
          : null
    )
...
export const duplicateCols = board => {
  return cols(board)
    .map(
      (col, index) =>
        cols(board).filter(
          (col2, index2) => index !== index2 && areIdentical(col, col2)
        ).length > 0
          ? index
          : null
    )
...
export const isPossibleMove = (board, rowIndex, columnIndex, value) => {
  ...
  if (
    threeOrMoreInARow(rows(board)[rowIndex]).length > 0 ||
  ...
  if (duplicateRows(board).length > 0 || duplicateCols(board) > 0) {
    board[rowIndex][columnIndex] = originalValue; // reset!
    return false;
  }
  board[rowIndex][columnIndex] = originalValue; // reset!
...
export const boardHasErrors = board => {
  ...
  if (rows.map(threeOrMoreInARow).filter(e => e.length > 0).length > 0)
    return true;
  if (columns.map(threeOrMoreInARow).filter(e => e.length > 0).length > 0)
    return true;
...
export const numSquaresFilled = (board, correctOnly = false) => {
  ...
  let filled = board.reduce((sum, row) => {
  ...
  return (
    filled -
    dupeCols / 2 * boardSize -
    dupeRows / 2 * boardSize -
    board
      .map(row => threeOrMoreInARow(row))
      .reduce((sum, dupes) => sum + dupes.length, 0) -
    cols(board)
      .map(col => threeOrMoreInARow(col))
      .reduce((sum, dupes) => sum + dupes.length, 0)
  );
};
...
export const fillBoard = (n = 6, solve = false) => {
  ...
  if (!solve) {
    removeRandomValuesFromBoard(board, 25);
  }
